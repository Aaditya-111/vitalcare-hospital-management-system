...
async function main() {
  ...
  // Note: We might want to clear users too, but be careful with admin users.
  // For this script, we'll just focus on doctors.
  ...
  for (const doctor of doctors) {
    const email = `${doctor.name.toLowerCase().replace(/\s+/g, '.')}@vitalcare.com`
